...
def normalize_idx(text: str, max_len: int = 128) -> str:
    ...
    if len(idx) > max_len:
        ...
        m = hashlib.md5(usedforsecurity=False)  # short non-cryptographic suffix to fit max_len
        m.update(rest)
        h = m.hexdigest()
...
def validate_idx(idx: str, min_len=1, max_len=128):
    ...
    if idx_normalized != idx:
        raise ValueError(f"idx must be slugify, maybe use normalize_idx()? {idx_normalized} != {idx}")
    if len(idx) < min_len:
...
def validate_sku(sku):
    """Validate a product SKU: stripped, non-empty, within length bounds, no forbidden chars."""
    min_len = 1
    max_len = 128
    forbidden_chars = [","]
    sku_normalized = normalize_sku(sku)
    if sku_normalized != sku:
        raise ValueError(f"sku must be striped, maybe use normalize_sku()? {sku_normalized} != {sku}")
...
def validate_ean(ean, min_len=8, max_len=16):
    ...
    if ean_normalized != ean:
        raise ValueError(f"ean must be lower and strip, maybe use normalize_ean()? {ean_normalized} != {ean}")
    if len(ean) < min_len:
...
def validate_url_key(url_key):
    ...
    if url_key != orig:
        raise Exception(
            f'ProductCategory "url_key" must be striped and lowered and slugified, maybe use normalize_url_key()? {url_key} != {orig}'
        )
